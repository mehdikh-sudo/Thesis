...
def parse_args():
    ...
    parser.add_argument(
        '--grad_clip', type=float, #default=1.0,
         default=0, help='gradient clipping max norm'
    )

    #early stopping # new   
    parser.add_argument(
        '--early_stop_patience', type=int, default=15, help='early stopping patience (epochs)'
    )
    #label smoothing arg # new
    parser.add_argument(
        '--label_smoothing', type=float, #default=0.1,
        default=0, help='label smoothing epsilon'
    )
    ...
    with tqdm(
        ...
        with torch.no_grad():
            ...
            for data, target, video_number, image_number in val_loader:
                ...
                # --- Fix: accumulate loss across sequences then divide,
                # so val_loss_metric gets ONE update per batch (same as train_loss). ---
                batch_val_loss = 0
                for sequence in range(args.sequence):
                    target_seq = multi_target[sequence].to(device)
                    output_seq = multi_output[sequence]
                    batch_val_loss += criterion(output_seq, target_seq)
                batch_val_loss /= args.sequence          # normalise: 1 value per batch
                val_loss_metric.update(batch_val_loss)   # single update — matches train

                # Second pass: build prediction & target lists (no loss computed here)
                for sequence in range(args.sequence):
                    target_seq = multi_target[sequence].to(device)
                    output_seq = multi_output[sequence]
            ...
            for i in eval_res:
                ...
                bert_k   += i[3]
                if args.dataset == "TVSum" or args.dataset=='TvSum_Rgb_Flow_Resnet' or args.dataset=='TvSum_Rgb_Flow'or args.dataset=='TVSum_RFR_matched_10class':
                    rho_k    += i[4]   # Spearman ρ (new)
                    tau_k    += i[5]   # Kendall τ-b (new)
            ...
            pd_val_loss.append(avg_val_loss)  # Store validation loss
            # ... add these three ...
...
def train(model, train_loader, optimizer, criterion, epoch, args):
    ...
    for batch_idx, (data, target, video_number, image_number) in enumerate(train_loader):
        ...
        for sequence in range(args.sequence):
            ...
            multi_loss += loss
        multi_loss /= args.sequence          # divide BEFORE logging and backward
        train_loss.update(multi_loss)        # one update per batch — same unit as val
        multi_loss.backward()
...
def train_net(args):
    ...
    print(args.dataset)
    print("Init...")
    train_loader, val_loader, In_target, val_In_target = build_dataloader(args)

    # --- Train criterion weights (from training set) ---
    total_target = len(train_loader) * args.batch_size * args.sequence
    A = total_target / (total_target - In_target)
    B = total_target / In_target

    # --- Val criterion weights (from val set, so CE scale matches) ---
    val_total = len(val_loader) * args.val_batch_size * args.sequence
    A_val = val_total / (val_total - val_In_target) if val_In_target < val_total else 1.0
    B_val = val_total / val_In_target if val_In_target > 0 else 1.0
    ...
    # Val criterion — uses val-set class weights so the loss scale is comparable
    val_criterion = nn.CrossEntropyLoss(weight=torch.FloatTensor([A_val, B_val])).to(device)
    ...
    while epoch < args.epochs:
        ...
        if (epoch + 1) % args.test_epochs == 0:
            val(model, val_loader, epoch, args, val_criterion)
        ...
        if args.dataset == "TVSum"  or args.dataset=='TvSum_Rgb_Flow_Resnet' or args.dataset=='TvSum_Rgb_Flow' or args.dataset=='TVSum_RFR_matched_10class':
            ...
            ddict = {'test_dataset':args.test_dataset,
                    'epoch': pd_epoch,
