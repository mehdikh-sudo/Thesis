...
def adjust_learning_rate(args, optimizer, epoch, batch_idx, data_nums, type):
    if epoch < args.warmup_epochs:
        ...
        # Linear warmup: lr goes from args.lr*0.1 (=0.003) to args.lr (=0.03).
        # lr_adj is a ratio in [0.1, 1.0] so that args.lr * lr_adj gives the right lr.
        # At start of warmup: lr_adj=0.1 → lr=0.003
        # At end of warmup:   lr_adj=1.0 → lr=0.03  (seamless handoff to cosine)
        initial_ratio = 0.1  # 0.003 / 0.03
        lr_adj = initial_ratio + (1.0 - initial_ratio) * (epoch / args.warmup_epochs)
