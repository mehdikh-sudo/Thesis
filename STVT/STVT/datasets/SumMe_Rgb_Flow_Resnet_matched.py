...
def SumMe_Rgb_Flow_Resnet_matched(args, distributed=False):
    class SumMe_Rgb_Flow_Resnet_matchedDataset(Dataset):
        global In_target
        In_target = 0
            ...
            with h5py.File(file_dir, "r") as f:
                ...
                for key in f.keys():
                    ...
                    if video_number in video_amount:
                        ...
                        ##added for 
                        # After line 39 in SumMe.py:
                        # remainder = downsample_image_number % patch_number

                        # # If last segment is incomplete, pad it by repeating last frame
                        # if remainder > 0:
                        #     last_frame = features[-1:]  # Shape: (1, 512)
                        #     padding_needed = patch_number - remainder
                        #     padding_frames = np.repeat(last_frame, padding_needed, axis=0)
                        #     features = np.concatenate([features, padding_frames], axis=0)
                        #     # Also pad labels
                        #     last_label = gtsummary[-1:]
                        #     padding_labels = np.repeat(last_label, padding_needed, axis=0)
                        #     gtsummary = np.concatenate([gtsummary, padding_labels], axis=0)
                        #     downsample_image_number = len(features)  # Update after padding
                        #     gonumber = int(downsample_image_number / patch_number)        
    ...
    train_data = SumMe_Rgb_Flow_Resnet_matchedDataset(file_dir=file_dir, video_amount=video_amount, F_In_target=True)
    train_loader = DataLoader(dataset=train_data, batch_size=args.batch_size, shuffle=True, drop_last=True)
    video_amount = test_arr
    test_data = SumMe_Rgb_Flow_Resnet_matchedDataset(file_dir=file_dir, video_amount=video_amount, F_In_target=False)
