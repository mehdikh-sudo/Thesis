...
def TVSum_Rgb_Flow_Resnet(args, distributed=False):
    class TVSum_Rgb_Flow_ResnetDataset(Dataset):
        global In_target
        In_target = 0
    ...
    file_dir = '/Users/mehdikhosravi/Master/Thesis/STVT-main/STVT/datasets/datasets/TVSum_Rgb_Flow_Resnet.h5' #stvt_i3d_tvsum
    video_amount = train_arr
    train_data = TVSum_Rgb_Flow_ResnetDataset(file_dir=file_dir, video_amount=video_amount, F_In_target=True)
    train_loader = DataLoader(dataset=train_data, batch_size=args.batch_size, shuffle=True, drop_last=True)
    video_amount = test_arr
    test_data = TVSum_Rgb_Flow_ResnetDataset(file_dir=file_dir, video_amount=video_amount, F_In_target=False)
