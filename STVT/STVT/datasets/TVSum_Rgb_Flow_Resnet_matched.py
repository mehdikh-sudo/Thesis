...
def TVSum_Rgb_Flow_Resnet_matched(args, distributed=False):
    class TVSum_Rgb_Flow_Resnet_matchedDataset(Dataset):
        global In_target
        In_target = 0
    ...
    file_dir = '/Users/mehdikhosravi/Master/Thesis/STVT-main/STVT/datasets/datasets/TVSum_Rgb_Flow_Resnet_matched.h5' #stvt_i3d_tvsum
    video_amount = train_arr
    train_data = TVSum_Rgb_Flow_Resnet_matchedDataset(file_dir=file_dir, video_amount=video_amount, F_In_target=True)
    train_loader = DataLoader(dataset=train_data, batch_size=args.batch_size, shuffle=True, drop_last=True)
    video_amount = test_arr
    test_data = TVSum_Rgb_Flow_Resnet_matchedDataset(file_dir=file_dir, video_amount=video_amount, F_In_target=False)
